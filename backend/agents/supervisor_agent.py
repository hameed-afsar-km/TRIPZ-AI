...
- Infer currency from origin (India→INR, USA→USD, etc.)
...
5. Currency (INR if from/in India, USD otherwise; infer from origin if possible)
...
{{"destination":"","origin":"","travel_dates":{{"start":"2025-06-01","end":"2025-06-10"}},"num_travelers":1,"budget":999999,"currency":"INR","preferences":["visit all places"],"duration_days":10,"confidence":0.9}}"""
...
async def supervisor_agent(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "error" in parsed:
        ...
        end_date = start_date + timedelta(days=fallback_duration)
